...
export function detectDisintermediation(text) {
  ...
  for (const pattern of DIRECT_PHONE_PATTERNS) {
    if (pattern.test(text)) {
      return {
        flagged: true,
        reason: "Terdeteksi nomor telepon atau kontak WhatsApp. Demi keamanan dan garansi dana escrow Bantuin, pertukaran kontak dianjurkan setelah pesanan resmi disepakati.",
      };
    }
  }
  ...
  for (const keyword of SOLICITATION_KEYWORDS) {
    if (lowerText.includes(keyword)) {
      return {
        flagged: true,
        reason: `Terdeteksi indikasi ajakan transaksi luar sistem ("${keyword}"). Gunakan rekening bersama (Escrow) resmi untuk memastikan garansi uang kembali.`,
      };
    }
  }
