...
export const MITRA_STORES = {
  "mitra-kamera": {
    ...
    securityDepositPolicy: "Wajib menitipkan 1 identitas asli (KTP/KTM) dan verifikasi akun Bantuin Escrow saat serah terima barang sewa.",
    catalog: [
      {
        id: "cam-1",
...
/**
 * Helper untuk mengambil data toko mitra berdasarkan ID
 */
export function getMitraStoreById(id) {
  if (!id) return MITRA_STORES["mitra-kamera"];
  if (MITRA_STORES[id]) return MITRA_STORES[id];
  if (MITRA_STORES[`mitra-${id}`]) return MITRA_STORES[`mitra-${id}`];
  if (id === "owner-1" || id === "owner-2") return MITRA_STORES["mitra-kamera"];
  if (id.toLowerCase().includes("kamera") || id.toLowerCase().includes("cam")) return MITRA_STORES["mitra-kamera"];
  if (id.toLowerCase().includes("laptop") || id.toLowerCase().includes("it")) return MITRA_STORES["mitra-laptop"];
  if (id.toLowerCase().includes("cetak") || id.toLowerCase().includes("print")) return MITRA_STORES["mitra-cetak"];
  if (id.toLowerCase().includes("sound") || id.toLowerCase().includes("audio")) return MITRA_STORES["mitra-sound"];
  return MITRA_STORES["mitra-kamera"];
}
