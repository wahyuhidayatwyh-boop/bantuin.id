...
export function extractKabupatenName(locationString, userRealLocation = null) {
  const str = String(locationString || userRealLocation?.city || "").trim();
  if (!str) {
...
export function isItemInKabupaten(item, activeCityName, activeCoords = null, maxRadiusKm = 35) {
  ...
  // 2. Pencocokan jika nama kota di dalam teks alamat
  if (normActive && itemAddress) {
    const cleanAddress = itemAddress.replace(/[\s\-_.]+/g, "");
