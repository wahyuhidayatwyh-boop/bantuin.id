...
export default function SewaSection() {
  const { rentals, activeKabupaten = "Indonesia", isItemInCurrentKabupaten } = useApp();

  const localRentals = (rentals || []).filter((item) =>
    isItemInCurrentKabupaten ? isItemInCurrentKabupaten(item) : true
  );
  const baseList = localRentals.length > 0 ? localRentals : (rentals || []);

  // Organic ranking of local rentals (Section Z)
  const displayedRentals = [...baseList]
    .sort((a, b) => {
      const scoreA = (Number(a.ratingAvg || a.rating) || 0) * 10 + (Number(a.totalRentedCount) || 0) + (a.isVerifiedPartner ? 10 : 0);
      const scoreB = (Number(b.ratingAvg || b.rating) || 0) * 10 + (Number(b.totalRentedCount) || 0) + (b.isVerifiedPartner ? 10 : 0);
        ...
        {displayedRentals.length > 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-4 md:gap-5">
            {displayedRentals.map((item) => (
              <div
                key={item.id}
                className="bg-white rounded-xl sm:rounded-[22px] border border-slate-200/90 overflow-hidden shadow-[0_4px_16px_rgba(0,0,0,0.03)] hover:shadow-[0_12px_30px_rgba(22,131,255,0.08)] hover:-translate-y-1 transition-all duration-200 flex flex-col justify-between"
              >
                <div>
                  ...
                  <div className="relative aspect-[4/3] w-full bg-slate-100 overflow-hidden">
                    ...
                    {item.isVerifiedPartner && (
                      <div className="absolute top-2 right-2 sm:top-3 sm:right-3 bg-emerald-50/95 backdrop-blur-md border border-emerald-200 px-1.5 sm:px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] font-bold text-emerald-700 flex items-center gap-1 shadow-2xs">
                        <ShieldCheck className="w-3 h-3 text-emerald-600" />
                        <span className="hidden sm:inline">Mitra Terverifikasi</span>
