      ...
      <div className="p-2.5 sm:p-4 flex flex-col justify-between flex-1">
        <div>
          ...
          <Link href={isRental ? `/sewa/${item.id}` : `/jasa/${item.id}`} className="block group">
            <h3 className="font-bold text-xs sm:text-[14px] text-slate-900 line-clamp-2 min-h-[32px] sm:min-h-[40px] leading-snug group-hover:text-[#1683FF] transition mb-1 sm:mb-1.5">
              {item.title}
            </h3>
          </Link>

          {/* Provider / Owner name + Distance info */}
          <div className="flex items-center justify-between gap-1 text-[11px] sm:text-xs text-slate-500 mb-1.5 sm:mb-2">
        ...
        <div className="pt-2 sm:pt-2.5 border-t border-slate-100 flex items-center justify-between gap-1 mt-auto">
          <div className="flex items-center gap-0.5 sm:gap-1 text-[11px] sm:text-xs font-semibold text-slate-800">
          ...
          <div className="text-right">
            <div className="text-[9px] sm:text-[10px] text-slate-400 font-medium uppercase tracking-wider">
              {isRental ? "Sewa / hari" : "Mulai"}
            </div>
            <div className="font-black text-xs sm:text-sm md:text-[15px] text-[#1683FF] sm:text-slate-900">
