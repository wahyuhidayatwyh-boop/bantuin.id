...
export default function OrderRoomWorkspacePage() {
  const params = useParams();
  const router = useRouter();
  const initialRoomId = params?.id || "order-room-101";
  ...
  const selectedRoom = orderRooms.find((r) => r.id === selectedRoomId) || orderRooms.find((r) => r.id === initialRoomId) || orderRooms[0];
  ...
  useEffect(() => {
    if (!orderRooms.some((r) => r.id === selectedRoomId) && orderRooms.length > 0) {
  ...
  const isOnline = selectedRoom?.mode === "online" || selectedRoom?.category?.toLowerCase().includes("desain") || selectedRoom?.category?.toLowerCase().includes("tugas");
  const isInquiry = selectedRoom?.stage === "inquiry" || selectedRoom?.orderStatus === "inquiry" || selectedRoom?.id?.startsWith("inquiry-");
          ...
          <div className="min-w-0 flex items-center gap-1.5 sm:gap-2">
            {isInquiry ? (
              ...
              <span className="text-[10px] font-bold px-2 py-0.5 bg-blue-50 text-[#1683FF] border border-blue-200 rounded-md shrink-0 flex items-center gap-1">
                <Lock className="w-3 h-3 text-[#1683FF] shrink-0" />
